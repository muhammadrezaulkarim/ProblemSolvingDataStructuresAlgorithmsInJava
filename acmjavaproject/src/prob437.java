		...
		//	   Sieve(20000000);
			...
			while(i<count)
			{
				...
				z=sc.nextInt();
					...
					// consider first value as width, second=depth, third=height
					bk=new block(y,z,x);
					blist.add(bk);
	...
	public void SortList()
	{
		Collections.sort(blist, new Comparator<block>(){
		    ...
		    /*	if((b1.x>b2.x && b1.y>b2.y) || (b1.x==b2.x && b1.y>b2.y))
		    	 ...
		    	 return( b1.x*b1.y >= b2.x*b2.y )?0:1;
		     }
...
int i=0,j=0, maxj=0;
	...
	/*		if(i>0)
			...
			*/
			...
			for(j=0;j<i;j++)
			{
		if((blist.get(i).x<blist.get(j).x && blist.get(i).y<blist.get(j).y))
